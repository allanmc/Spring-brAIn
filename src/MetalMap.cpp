...
*/
	...
	MetalMapHeight = Callback->GetMap()->GetHeight() / 2;
	MetalMapWidth = Callback->GetMap()->GetWidth() / 2;

	TotalCells = MetalMapHeight * MetalMapWidth;
	XtractorRadius = int(Callback->GetMap()->GetExtractorRadius( *Metal ) / 16);
...
void MetalMap::GetMetalPoints()
{
	...
	vector<unsigned char> metalMapArray = Callback->GetMap()->GetResourceMapRaw( *Metal );
	double TotalMetalDouble  = 0;

	for (int i = 0; i < TotalCells; i++) {
		// count the total metal so you can work out an average of the whole map
		TotalMetalDouble +=  MexArrayA[i] = metalMapArray.at(i);
	}
	...
	for (int a = 0; a < MaxSpots; a++) {
		...
		if (!Stopme) {
			...
			BufferSpot.y = TempMetal * (Callback->GetMap()->GetMaxResource( *Metal) ) * MaxMetal / 255;
